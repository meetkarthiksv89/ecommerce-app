...
class SignUpViewController: UIViewController{
    ...
    let loginView = PInputView(frame: .zero, noOfTextField: 3, title: "Sign Up",buttonTitleText: "Sign Up",placeHolder: ["Name","Email","Password"])
    ...
    func signUp(credentials:[String]) {
        
        let username = credentials[0]
        let email = credentials[1]
        let password = credentials[2]
        
        if username == "" {loginView.errorAtTextField(atIndex: 0)}
        if email == "" || !email.isValidEmail {loginView.errorAtTextField(atIndex: 1)}
        if password == "" || !password.isValidPassword {loginView.errorAtTextField(atIndex: 2)}
...
extension SignUpViewController: PInputViewDelegate {
    
    func backButtonTapped() {
        navigationController?.popViewController(animated: true)
    }
    func buttonTapped(values: [String]) {
        signUp(credentials: values)
    }
}
